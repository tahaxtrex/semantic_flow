...
### Silent Schema Defaults Corrupt Scores
...
### Wasteful Full-Rubric Prompting
...
### Output Overload (Zero-Shot Cognitive Load)
- **Location:** `src/evaluator.py` (`_build_prompt`)
...
### Pointless Evaluation of Non-Instructional Segments
- **Location:** `src/evaluator.py` (`_build_prompt`)
...
- **Arbitrary Sentence Chunking:** `_chunk_text` in `segmenter.py` splits strings strictly by `max_chars` down to the exact sentence. This can break context mid-paragraph. A sliding window or paragraph-respecting chunker would perform better.
- **Brittle Header Detection:** `segmenter.py` uses `>= 1.4x body median` font size to detect headers. This breaks on textbooks that use bold text, small caps, or color to denote headers rather than strict size increments.
- **Blind Retries:** `_retry_call` in `evaluator.py` retries on *any* exception. Standard `ValidationError` outputs from LLMs or token limit exceptions shouldn't be retried blindly with exponential backoff; they will just fail again deterministically. Catch `429` (Rate Limit) and `503` (Service Unavailable) specifically for backoff.
- **Vague Null Handling:** The schema relies on string matches like `"Unknown"`. It is better to use `Optional[str] = None` so downstream pipelines cleanly encounter nulls instead of string literals.
...
|---|-------|----------|----------|---------------|
| 1 | Model Cascading Invalidates Research Validity | Validity | Critical | `src/evaluator.py` |
| 2 | Silent Schema Defaults Corrupt Scores | Extraction | Critical | `src/evaluator.py` & `src/models.py` |
| 3 | Destructive Segment Merging | Segmentation | Critical | `src/segmenter.py` |
| 4 | Wasteful Full-Rubric Prompting | Cost | Critical | `src/evaluator.py` |
| 5 | Output Overload (Zero-Shot Cognitive Load) | Accuracy | Critical | `src/evaluator.py` |
| 6 | Data Loss in Table Extraction | Extraction | Significant | `src/segmenter.py` |
| 7 | Incomplete Metadata Extraction | Extraction | Significant | `src/metadata.py` |
| 8 | Pointless Evaluation of Non-Instructional Segments | Cost | Significant | `src/evaluator.py` |
| 9 | Unweighted Score Aggregation | Validity | Significant | `src/aggregator.py` |
| 10 | Arbitrary Sentence Chunking | Segmentation | Minor | `src/segmenter.py` |
| 11 | Brittle Header Detection | Segmentation | Minor | `src/segmenter.py` |
| 12 | Blind Retries on Deterministic Errors | Cost | Minor | `src/evaluator.py` |
| 13 | Vague Null Handling | Extraction | Minor | `src/models.py` |
